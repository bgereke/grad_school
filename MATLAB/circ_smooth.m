function [c,ww] = circ_smooth(varargin)
%SMOOTH  Smooth data.
...
if ~xreps && ~any(ynan)
    ...
    cx = filter(ones(width,1)/width,1,yx);
    cxbegin = cumsum(yx(1:width-2));
    cxbegin = cxbegin(1:2:end)./(1:2:(width-2))';
    cxend = cumsum(yx(n:-1:n-width+3));
    cxend = cxend(end:-2:1)./(width-2:-2:1)';
    cx = [cxbegin;cx(width:end);cxend];
    cy = filter(ones(width,1)/width,1,yy);
    cybegin = cumsum(yy(1:width-2));
    cybegin = cybegin(1:2:end)./(1:2:(width-2))';
    cyend = cumsum(yy(n:-1:n-width+3));
    cyend = cyend(end:-2:1)./(width-2:-2:1)';
    cy = [cybegin;cy(width:end);cyend];
    c = atan2(cy,cx);
end
% elseif ~xreps
