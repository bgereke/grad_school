...
int SkipCheetahNTHeader(FILE *fp, double *bv)
{
  ...
  //extract the bv multiplier
  for(ii=0; ii<10; ii++) fgets(NULL, 256, fp);
  fgets(bvline,256,fp);
  bvst = strtok(bvline," 	");
  bvst = strtok(NULL," 	");
  for(ii=0; ii<4; ii++)
  {
	if(bvst!=NULL)	bv[ii] = atof(bvst)* (double) pow(10.0,6.0);
  }
...
int GetNumberOfSpikes(char* fn){
  ...
  SkipCheetahNTHeader(fp, bv);     // Skip standard Neuralynx header if present (Cheetah versions >= 1.3)
...
void ReadTT(char* fn, int nSpikes, double *t, double *wv){
  ...
  for (int i = 0; i < nSpikes; i++){
    ...
    for (int j = 0; j<4; j++)
      for(int k = 0; k<32; k++)
	wv[i + nSpikes*j + nSpikes*4*k] = (double) wv0[j + 4*k] * bv[j];  
  }
  ...
  while(i < n_records_to_get)
    {
      ...
      for (int j = 0; j<4; j++)
	for(int k = 0; k<32; k++)
	  wv[i + n_records_to_get*j + n_records_to_get*4*k] = (double) wv0[j + 4*k]*bv[j];	
      i++;
    }
  fclose(fp);	
}	
