...
% Analysis, 2014.
%
% Test piecewise wave shape functions with TV-norm minimization
%
% By Haizhao Yang
...
if(1)
    ...
    f1 = zeros(1,N);
    am = 1+0.4*sin(4*pi*x);
    f1 = am.*gen_shape(F1*xx,3);
    yy = x + 2*amp*cos(4*pi*x);
    f2 = zeros(1,N);
    bm = 1-0.3*sin(2*pi*x);
    f2 = bm.*gen_shape(F2*yy,7);
...
if (1)
    ...
    pic = figure; hold on; plot(x,ins_freq(2,:),'r'); plot(x,(1+amp*2*pi*cos(2*pi*x))*F1,'b');
    plot(x,ins_freq(1,:),'r'); plot(x,(1-amp*8*pi*sin(4*pi*x))*F2,'b');axis square; 
    ...
    Nit = 100;
    for cnt = 1:num_group
        [general_shape_comp(cnt,:), cost] = TVD_mm(general_shape_comp(cnt,:), lam, Nit);
    end
    pic = figure; hold on; plot(x(1:300),general_shape_comp(2,1:300),'r'); plot(x(1:300),f1(1:300),'b');axis square;title('1st mode');
    pic = figure; hold on; plot(x(1:300),general_shape_comp(1,1:300),'r'); plot(x(1:300),f2(1:300),'b');axis square;title('2nd mode');
