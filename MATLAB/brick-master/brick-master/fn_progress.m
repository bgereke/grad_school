...
if ischar(varargin{1})
    ...
    % start timers
    x(1).timer = tic;
    x(1).lastdisp = nsum(tic)-1e6; % trick: save the time it was 1s before, so that next display attempt will detect a delay > 1s and will not cancel the display
else
    ...
    if calllevel && toc(uint64(x(1).lastdisp))<.1 && i~=x(1).max, return, end
    % display
    if ~calllevel
