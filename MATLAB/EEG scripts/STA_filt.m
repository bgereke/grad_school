...
for ii = 1:numsessions
    ...
    for ss=1:size(dirInfo,1)
        if dirInfo(ss).isdir
            if strcmp(dirInfo(ss).name,strcat('STA_filt_plots','\'))
                found = 1;
            end
        end
    end
    if found==0
        mkdir(sessions{ii},strcat('STA_filt_plots','\'));
    end
    ...
    for jj=1:numcells
        ...
        filename = sprintf('%s%s%s%s',sessions{ii},strcat('STA_filt_plots','\'),cells{jj}(1:end-2),'.mat');
        save(filename,'-struct','data');
        plot((1:size(eeg_epochs_grouped,1))/Fs*1000,mean(eeg_epochs_grouped,2));
        xlabel('time (ms)'); ylabel('uV')
        title(['Spike Triggered LFP: ',tfile]);
        figImage = sprintf('%s%s%s%s',sessions{ii},strcat('STA_filt_plots','\'),cells{jj}(1:end-2),'.fig');
        bmpImage = sprintf('%s%s%s%s',sessions{ii},strcat('STA_filt_plots','\'),cells{jj}(1:end-2),'.bmp');
        epsImage = sprintf('%s%s%s%s',sessions{ii},strcat('STA_filt_plots','\'),cells{jj}(1:end-2),'.eps');
