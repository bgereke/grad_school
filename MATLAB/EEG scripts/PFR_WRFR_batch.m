...
function [PFR] = PFR_WRFR_batch(inFile,freqVec,phasebins)
...
for ii = 1:numsessions
    ...
    for kk=1:size(dirInfo,1)
        if dirInfo(kk).isdir
            if strcmp(dirInfo(kk).name,strcat('PFR_WRFR','\'))
                found = 1;
            end
        end
    end
    if found==0
        mkdir(sessions{ii},strcat('PFR_WRFR','\'));
    end
...
%     y = y - avref;
    clear samples
    [PFR] = PFR_WRFR(x,y,freqVec,5,Fs,phasebins);
%     PFR = PFR.^(0.5);
    data.freq = freqVec; data.phasebins = phasebins; data.pfr = PFR;   %save data as struct
    filename = sprintf('%s%s%s%s',sessions{ii},strcat('PFR_WRFR','\'),channels{1}(1:end-4),'_',channels{2}(1:end-4),'.mat');
    save(filename,'-struct','data');
    figure;
    imagesc(phasebins(1:end-1),freqVec,PFR); axis xy; colorbar
    xlabel('phase (rads)');ylabel('frequency (Hz)');
    title(strcat('Average Theta Phase Triggered WRFR: ',channels{1}(1:end-4),'x',channels{2}(1:end-4)));
    figImage = sprintf('%s%s%s%s',sessions{ii},strcat('PFR_WRFR','\'),channels{1}(1:end-4),'_',channels{2}(1:end-4),'.fig');
    bmpImage = sprintf('%s%s%s%s',sessions{ii},strcat('PFR_WRFR','\'),channels{1}(1:end-4),'_',channels{2}(1:end-4),'.bmp');
    epsImage = sprintf('%s%s%s%s',sessions{ii},strcat('PFR_WRFR','\'),channels{1}(1:end-4),'_',channels{2}(1:end-4),'.eps');
