...
function [PLV] = PLVspec(inFile,freqVec,width)
...
for ii = 1:numsessions
    ...
    for kk=1:size(dirInfo,1)
        if dirInfo(kk).isdir
            if strcmp(dirInfo(kk).name,strcat('PLVspec','\'))
                found = 1;
            end
        end
    end
    if found==0
        mkdir(sessions{ii},strcat('PLVspec','\'));
    end
...
%         Ref = bv*samples;
%         ch_X = ch_X + Ref;
        [PLV,timeVec,freqVec] = traces2PLV(ch_X,ch_Xp,freqVec,Fs,width);
        data.freq = freqVec; data.PLV = PLV; data.time = timeVec;   %save data as struct
        filename = sprintf('%s%s%s%s',sessions{ii},strcat('PLVspec','\'),channels{jj}(1:end-4),'.mat');
        save(filename,'-struct','data');
        figure(1);
        plot(freqVec,PLV);
        xlabel('Frequency (Hz)');ylabel('Phase Locking');
        title(strcat('Phase Locking Spectrum: ',file));
        figImage = sprintf('%s%s%s%s',sessions{ii},strcat('PLVspec','\'),channels{jj}(1:end-4),'.fig');
        bmpImage = sprintf('%s%s%s%s',sessions{ii},strcat('PLVspec','\'),channels{jj}(1:end-4),'.bmp');
        epsImage = sprintf('%s%s%s%s',sessions{ii},strcat('PLVspec','\'),channels{jj}(1:end-4),'.eps');
