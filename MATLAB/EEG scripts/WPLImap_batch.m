function [WPLImaps] = WPLImap_batch(inFile,freqVec,startmin,endmin,width) 
...
for ii = 1:numsessions
    ...
    for kk=1:size(dirInfo,1)
        if dirInfo(kk).isdir
            if strcmp(dirInfo(kk).name,strcat('WPLImaps','\'))
                found = 1;
            end
        end
    end
    if found==0
        mkdir(sessions{ii},strcat('WPLImaps','\'));
    end
...
%         P{ii} = mean(P{ii,jj});
        ...
        for f = 1:length(freqVec)
            WPLImaps{ii} = WPLImaps{ii} + WPLImap(P{ii}(f,:),Px,Py,h,mapAxis);
        end
        WPLImaps{ii} = WPLImaps{ii}/length(freqVec);
        % Remove unvisited parts of the box from the map
        if length(WPLImaps{ii})>1
            WPLImaps{ii}(timeMaps{ii}==0) = NaN;
        end
        
        data.P = P{ii}; data.freq = freqVec; data.WPLImaps = WPLImaps(ii);   %save data as struct
        data.Px = Px; data.Py = Py; data.ts = ts; data.x = x; data.y = y; data.t = t;
        filename = sprintf('%s%s%s%s',sessions{ii},strcat('WPLImaps','\'),channels{jj}(1:end-4),'x',channels{jj+1}(1:end-4),'_',num2str(min(freqVec)),'-',num2str(max(freqVec)),'Hz','10.mat');
        save(filename,'-struct','data');        
        disp('Plot maps and store them to files');    
        close all; figure(1);colormap(hot(500));
        imagesc(mapAxis,mapAxis,WPLImaps{ii},[min(min(WPLImaps{ii})) max(max(WPLImaps{ii}))]); axis xy; colorbar;
        c = colorbar; ylabel(c,'debiased WPLI');
%         imagesc(mapAxis,mapAxis,LFPmaps{ii,jj},[0 1]); axis xy;
        ...
        title(['WPLI Map: ',num2str(min(freqVec)),'-',num2str(max(freqVec)),' Hz: ',channels{jj}(1:end-4),'x',channels{jj+1}(1:end-4)]);
        figImage = sprintf('%s%s%s%s',sessions{ii},strcat('WPLImaps','\WPLImap_'),channels{jj}(1:end-4),'x',channels{jj+1}(1:end-4),'_',num2str(min(freqVec)),'-',num2str(max(freqVec)),'Hz','10.fig');
        bmpImage = sprintf('%s%s%s%s',sessions{ii},strcat('WPLImaps','\WPLImap_'),channels{jj}(1:end-4),'x',channels{jj+1}(1:end-4),'_',num2str(min(freqVec)),'-',num2str(max(freqVec)),'Hz','10.bmp');
...
function map = WPLImap(P,Px,Py,h,mapAxis)
invh = 1/h;
...
outsum = P*gaussian_kernel((Px-i),(Py-j),invh);
outsumW = abs(P)*gaussian_kernel((Px-i),(Py-j),invh);
% outssq = (P.^2)*gaussian_kernel((Px-i),(Py-j),invh);
% p = abs(outsum^2 - outssq)/(outsumW^2 - outssq); % do the pairwise thing in a handy way
% p = p.^(0.5);
p = (outsum./outsumW).^2;
