function [noPi] = traces2noPi(x,ref,freqVec,Fs,width);
...
noPi = zeros(length(freqVec),size(x,2));

for j=1:length(freqVec)
    Wx(j,:) = energyvec(freqVec(j),detrend(x),Fs,width);
    Wy(j,:) = energyvec(freqVec(j),detrend(ref),Fs,width);
end

for j=1:length(freqVec)
    noPi(j,:) = xspec(freqVec(j),Wx(j,:),Wy(j,:),Fs,width);
end

phase = angle(noPi);
noPi = (2*abs(noPi).*cos(phase/2)/Fs).^2;
