...
for ii = 1:numsessions
    ...
    for kk=1:size(dirInfo,1)
        if dirInfo(kk).isdir
            if strcmp(dirInfo(kk).name,strcat('trace_filt_plots','\'))
                found = 1;
            end
        end
    end
    if found==0
        mkdir(sessions{ii},strcat('trace_filt_plots','\'));
    end
    ...
    for jj=1:numchannels
        ...
        data.pass_idx = pass_idx; data.pass_times = pass_times; data.ch = ch_X;   %save data as struct
        filename = sprintf('%s%s%s%s',sessions{ii},strcat('trace_filt_plots','\'),channels{jj}(1:end-4),'.mat');
        save(filename,'-struct','data');
        figure(1);
        for kk = 1:size(pass_idx,1)
            
            plot((1:(pass_idx(kk,2)-pass_idx(kk,1)+1))/Fs,ch_X(pass_idx(kk,1):pass_idx(kk,2)));
            xlabel('time (sec)'); ylabel('uV'); title(strcat('Trace: ',file,': ',num2str(kk)));
            figImage = sprintf('%s%s%s%s',sessions{ii},strcat('trace_filt_plots','\'),channels{jj}(1:end-4),'_trace_',num2str(kk),'.fig');
            bmpImage = sprintf('%s%s%s%s',sessions{ii},strcat('trace_filt_plots','\'),channels{jj}(1:end-4),'_trace_',num2str(kk),'.bmp');
            epsImage = sprintf('%s%s%s%s',sessions{ii},strcat('trace_filt_plots','\'),channels{jj}(1:end-4),'_trace_',num2str(kk),'.eps');
