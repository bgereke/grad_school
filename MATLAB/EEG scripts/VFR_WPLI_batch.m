...
function [VFR] = VFR_WPLI_batch(inFile,freqVec)
...
for jj = 1:numchannels-1
    for kk = jj+1:numchannels
        ...
        filename = sprintf('%s%s%s%s',strcat('VFR_WPLI','\'),channels{jj}(1:end-4),'_',channels{kk}(1:end-4),'.mat');
        save(filename,'-struct','data');
        figure(1)
        imagesc(vbins,freqVec,VFR); axis xy; colormap hot; colorbar
        xlabel('running speed (cm/sec)');ylabel('frequency (Hz)');
        title(strcat('Velocity Frequency WPLI: ',channels{jj}(1:end-4),'x',channels{kk}(1:end-4)));
        figImage = sprintf('%s%s%s%s',strcat('VFR_WPLI','\'),channels{jj}(1:end-4),'_',channels{kk}(1:end-4),'.fig');
        bmpImage = sprintf('%s%s%s%s',strcat('VFR_WPLI','\'),channels{jj}(1:end-4),'_',channels{kk}(1:end-4),'.bmp');
        ...
        xlabel('running speed (cm/sec)');ylabel('frequency (Hz)');
        title(strcat('Change in VFR WPLI: ',channels{jj}(1:end-4),'x',channels{kk}(1:end-4)));
        figImage = sprintf('%s%s%s%s',strcat('mVFR_WPLI','\'),channels{jj}(1:end-4),'_',channels{kk}(1:end-4),'.fig');
        bmpImage = sprintf('%s%s%s%s',strcat('mVFR_WPLI','\'),channels{jj}(1:end-4),'_',channels{kk}(1:end-4),'.bmp');
        saveas(gcf,figImage,'fig');
        saveas(gcf,bmpImage,'bmp');
        data.freq = freqVec; data.vbins = vbins; data.mvfr = mVFR; data.vfr = [];   %save data as struct
        filename = sprintf('%s%s%s%s',strcat('mVFR_WPLI','\'),channels{jj}(1:end-4),'_',channels{kk}(1:end-4),'.mat');
        save(filename,'-struct','data');
        figure(3)
        imagesc(vbins,freqVec,sVFR,[-c c]); axis xy; colormap jet; colorbar
        xlabel('running speed (cm/sec)');ylabel('frequency (Hz)');
        title(strcat('Significant Change in VFR WPLI: ',channels{jj}(1:end-4),'x',channels{kk}(1:end-4)));
        figImage = sprintf('%s%s%s%s',strcat('sVFR_WPLI','\'),channels{jj}(1:end-4),'_',channels{kk}(1:end-4),'_sig.fig');
        bmpImage = sprintf('%s%s%s%s',strcat('sVFR_WPLI','\'),channels{jj}(1:end-4),'_',channels{kk}(1:end-4),'_sig.bmp');
        saveas(gcf,figImage,'fig');
        saveas(gcf,bmpImage,'bmp');
        data.freq = freqVec; data.vbins = vbins; data.svfr = sVFR; data.mvfr = [];   %save data as struct
        filename = sprintf('%s%s%s%s',strcat('sVFR_WPLI','\'),channels{jj}(1:end-4),'_',channels{kk}(1:end-4),'.mat');
        save(filename,'-struct','data');  
        figure(4)
        c = prctile(reshape(zVFR,size(zVFR,1)*size(zVFR,2),1),95);
        imagesc(vbins,freqVec,zVFR,[-c c]); axis xy; colormap jet; colorbar
        xlabel('running speed (cm/sec)');ylabel('frequency (Hz)');
        title(strcat('Z-Scored Change in VFR WPLI: ',channels{jj}(1:end-4),'x',channels{kk}(1:end-4)));
        figImage = sprintf('%s%s%s%s',strcat('zVFR_WPLI','\'),channels{jj}(1:end-4),'_',channels{kk}(1:end-4),'.fig');
        bmpImage = sprintf('%s%s%s%s',strcat('zVFR_WPLI','\'),channels{jj}(1:end-4),'_',channels{kk}(1:end-4),'.bmp');
        saveas(gcf,figImage,'fig');
        saveas(gcf,bmpImage,'bmp');
        data.freq = freqVec; data.vbins = vbins; data.mvfr = zVFR; data.svfr = [];   %save data as struct
        filename = sprintf('%s%s%s%s',strcat('zVFR_WPLI','\'),channels{jj}(1:end-4),'_',channels{kk}(1:end-4),'.mat');
        save(filename,'-struct','data');
    end
end
