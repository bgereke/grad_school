...
function [GIMt] = PFR_GIM_batch_rats(inFile,kappa,numperms)
...
for kk=1:size(dirInfo,1)
    if dirInfo(kk).isdir
        if strcmp(dirInfo(kk).name,strcat('PFR_MC','\'))
            found = 1;
        end
    end
end
if found==0
    mkdir(cd,strcat('PFR_MC','\'));
end
...
% zidx = find(zscore(abs(Th))>1);
% fCS = fCS(:,zidx);
% V = V(zidx);
vbins = linspace(-pi,pi,100);
pMC = zeros(length(freqVec),length(vbins),numperms);
start = round(linspace(100,length(V)-100,numperms));
delta = angle(exp(1i*repmat(V,1,length(vbins))).*conj(exp(1i*repmat(vbins,length(V),1))));
W = exp(kappa*cos(delta));
D = ones(length(freqVec),length(V))*exp(kappa*cos(delta));
...
figImage = sprintf('%s%s%s%s',strcat('PFR_MC','\PFR_MC'),'.fig');
bmpImage = sprintf('%s%s%s%s',strcat('PFR_MC','\PFR_MC'),'.bmp');
saveas(gcf,figImage,'fig');
saveas(gcf,bmpImage,'bmp');

data.zMC = zMC; data.freqVec = freqVec; data.vbins = vbins;
filename = sprintf('%s%s%s%s',strcat('PFR_MC','\'),'data.mat');
