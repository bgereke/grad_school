...
function [GIMt] = VFR_GIM_batch_rats(inFile,sigma,numperms)
...
for kk=1:size(dirInfo,1)
    if dirInfo(kk).isdir
        if strcmp(dirInfo(kk).name,strcat('VFR_MC','\'))
            found = 1;
        end
    end
end
if found==0
    mkdir(cd,strcat('VFR_MC','\'));
end
...
figImage = sprintf('%s%s%s%s',strcat('VFR_MC','\VFR_MC'),'.fig');
bmpImage = sprintf('%s%s%s%s',strcat('VFR_MC','\VFR_MC'),'.bmp');
saveas(gcf,figImage,'fig');
saveas(gcf,bmpImage,'bmp');

data.zMC = zMC; data.freqVec = freqVec; data.vbins = vbins;
filename = sprintf('%s%s%s%s',strcat('VFR_MC','\'),'data.mat');
