...
function [GIMt] = VFR_GIM_batch(inFile,sigma,numperms)
...
for kk=1:size(dirInfo,1)
    if dirInfo(kk).isdir
        if strcmp(dirInfo(kk).name,strcat('VFR_GIM','\'))
            found = 1;
        end
    end
end
if found==0
    mkdir(cd,strcat('VFR_GIM','\'));
end
...
vmax = min([vmax 30]);
vbins = 0:0.5:vmax;
MC = zeros(length(freqVec),length(vbins),numchannels,numchannels);
pMC = zeros(length(freqVec),length(vbins),numchannels,numchannels,numperms);
...
for jj = 1:numchannels
    for kk = jj:numchannels    
        MC(:,:,jj,kk) = fCS(:,:,jj,kk)*W./D;
        for p = 1:numperms
            pMC(:,:,jj,kk,p) = [fCS(:,start(p):end,jj,kk) fCS(:,1:start(p)-1,jj,kk)]*W./D;
        end
    end
end
toc
...
zGIM = (GIM - mean(pGIM,3))./std(pGIM,0,3);
data.GIM = GIM;clear GIM

close all

maxcol = max(max(data.GIM(freqVec>6,:)));
mincol = min(min(data.GIM(freqVec>6,:)));
subplot(2,1,1)
imagesc(vbins,freqVec,data.GIM,[mincol maxcol]);
axis xy;axis square;colormap hot;hold off
xlabel('running speed (cm/sec)');ylabel('frequency (Hz)');
cbfreeze
freezeColors
cblabel('GIM')

col = max(max(abs(zGIM(freqVec>6,:))));
subplot(2,1,2)
imagesc(vbins,freqVec,zGIM,[-col col]);
axis xy;axis square;colormap jet;colorbar
xlabel('running speed (cm/sec)');ylabel('frequency (Hz)');
cblabel('z-score from null')

figImage = sprintf('%s%s%s%s',strcat('VFR_GIM','\VFR_GIM'),'.fig');
bmpImage = sprintf('%s%s%s%s',strcat('VFR_GIM','\VFR_GIM'),'.bmp');
saveas(gcf,figImage,'fig');
saveas(gcf,bmpImage,'bmp');

data.zGIM = zGIM; data.freqVec = freqVec; data.vbins = vbins;
filename = sprintf('%s%s%s%s',strcat('VFR_GIM','\'),'data.mat');
