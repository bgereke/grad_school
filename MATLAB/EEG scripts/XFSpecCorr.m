...
function [Corr] = XFSpecCorr(inFile,freqVec,width)
...
for ii = 1:numsessions
    ...
    for kk=1:size(dirInfo,1)
        if dirInfo(kk).isdir
            if strcmp(dirInfo(kk).name,strcat('Spectral_XFCorrelation_plots','\'))
                found = 1;
            end
        end
    end
    if found==0
        mkdir(sessions{ii},strcat('Spectral_XFCorrelation_plots','\'));
    end
...
%         ch_X = ch_X + Ref;
        ...
        R = zeros(length(freqVec),length(freqVec));

        for i = 1:length(freqVec)
            for j = 1:length(freqVec)
                r = corrcoef(TFR1(i,:),TFR2(j,:));
                R(i,j) = r(1,2);                
            end
        end

        data.freq = freqVec; data.coh = R;   %save data as struct
        filename = sprintf('%s%s%s%s',sessions{ii},strcat('Spectral_XFCorrelation_plots','\'),channels{jj}(1:end-4),'_',channels{jj+1}(1:end-4),'.mat');
        save(filename,'-struct','data');
        figure(1);
        imagesc(freqVec,freqVec,R); axis xy; colorbar
        xlabel('Frequency (Hz)');ylabel('Frequency (Hz)');
        title(strcat('Spectral XF Correlation: ',sessions{ii},channels{jj}(1:end-4),'_',channels{jj+1}(1:end-4)));
        figImage = sprintf('%s%s%s%s',sessions{ii},strcat('Spectral_XFCorrelation_plots','\'),channels{jj}(1:end-4),'_',channels{jj+1}(1:end-4),'.fig');
        bmpImage = sprintf('%s%s%s%s',sessions{ii},strcat('Spectral_XFCorrelation_plots','\'),channels{jj}(1:end-4),'_',channels{jj+1}(1:end-4),'.bmp');
        epsImage = sprintf('%s%s%s%s',sessions{ii},strcat('Spectral_XFCorrelation_plots','\'),channels{jj}(1:end-4),'_',channels{jj+1}(1:end-4),'.eps');
