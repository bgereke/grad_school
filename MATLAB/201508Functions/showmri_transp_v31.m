function [hh,hii,clims]=showmri_transp_v3(mri,para,varargin);
...
if nargin>1
  ...
  if isfield(para,'dslice_shown');
    ...
    dslice_shown2=para.dslice_shown;
  end
  if isfield(para,'dslice_shown2');
        dslice_shown2=para.dslice_shown2;
  end
...
if isfield(para,'subplotstart');
   subplotstart=para.subplotstart;
end
 if isfield(para,'nsubplotrows');
   nsubplotrows=para.nsubplotrows;
 end
end
...
%                   mydipmarkersize = 1;
                  re{ng} = rectangle('Curvature', [1 1], ...
                      ...
                      'edgecolor', mydipcolor, 'linewidth', 1.5);
                else
...
%           kk=kk
      ...
      if nkori==3
          ...
          %set(h0,'position',P0);
          %nkori=nkori
         PP=[P(1)-.01+(nval-kk)*.12  P(2)+.01 .15*P(3) P(4)*.99];
      elseif nn==1 & nval<3
          %P0=get(h0,'position');
          %P0=[P0(1) P0(2) (1.1+(kk-1)*.2)*P0(3) P0(4)];
          %set(h0,'position',P0);
      ...
      elseif nn==1 & nval>2
          ...
          %set(h0,'position',P0);
          PP=[ P(1)+0.6*1.2*P(3) P(2) .15*1.2*P(3) P(4)*1.2];
      else
          PP=[ P(1)+0.65*1.2*P(3) P(2) .15*1.2*P(3) P(4)*1.2];
      end
      set(gca,'position',PP);
      set(gca,'xtick',[]);
