...
function [peakRate, pathCoord] = spike_angles(inFile,scale_max)
...
for ii = 1:counter
    ...
    t{ii} = t{ii}-min(t{ii});    
    for jj=1:numCells
        if length(spk_phase{ii,jj})/max(t{ii}) > 10
            continue
        end
        close all
        figure(1); hold on
        ...
        plot(t{ii},phase{ii},'-k');
        idx = zeros(length(spk_phase{ii,jj}),1);
        for s = 1:length(spk_phase{ii,jj})
            [Y,idx(s)] = min((phase{ii}-spk_phase{ii,jj}(s)).^2);
        end
        plot(t{ii}(idx),phase{ii}(idx),'.r');title(strcat('Cell ',num2str(jj),'-',F{jj}))
        xlabel('time (sec)');ylabel('track angle');
        
        bmpImage = sprintf('%s%s%s%s',sessions{ii},strcat('SpikeAngles','\'),F{jj}(1:end-2),'.bmp');
        epsImage = sprintf('%s%s%s%s',sessions{ii},strcat('SpikeAngles','\'),F{jj}(1:end-2),'.eps');
        f = getframe(gcf);
        [pic, cmap] = frame2im(f);
        ...
        for p=1:length(pbins)
           vmap(p) = mean(V(abs(angle(exp(1j*P).*conj(exp(1j*pbins(p)*ones(size(V))))))<=1/360*pi));
        end
        [~,im] = min(vmap); vth = 0.4;
%         spk_temp = S(abs(angle(exp(1j*S).*conj(exp(1j*pbins(im)*ones(size(S))))))>vth & SV>vmin);
        spk_temp = S(abs(angle(exp(1j*S).*conj(exp(1j*pbins(im)*ones(size(S))))))>0.4);
        %compute map on remaining spikes
        [map{ii,jj},~,cidx] = circle_map(spk_temp,P,T,h,fieldAxis); 
        %do field detection
        minb = nan; maxb = nan;pad = 0.1;
        for b = 1:length(fieldAxis)
            if cidx - b > 0
                bb = cidx - b;           
            else
                bb = length(fieldAxis) - b + cidx;                       
            end
            if map{ii,jj}(bb)<minr && map{ii,jj}(bb)<minp*map{ii,jj}(cidx)
                minb = fieldAxis(bb);
            end            
            if ~isnan(minb), break; end
        end
        for b = 1:length(fieldAxis)
            if cidx + b <= length(fieldAxis)
                bb = cidx + b;
            else
                bb = cidx + b - length(fieldAxis);                        
            end      
            if map{ii,jj}(bb)<minr && map{ii,jj}(bb)<minp*map{ii,jj}(cidx)
                maxb = fieldAxis(bb);
            end
        ...
        field_id{ii,jj} = ones(length(spk_phase{ii,jj}),1);
        if minb < maxb
            field_id{ii,jj}(spk_phase{ii,jj}<minb) = 0;
            field_id{ii,jj}(spk_phase{ii,jj}>maxb) = 0;
        else
            field_id{ii,jj}((spk_phase{ii,jj}<minb & spk_phase{ii,jj}>maxb)) = 0;
        end
        
        figure(2);
        plot(t{ii},phase{ii},'-k'); hold on
        idx = zeros(length(spk_phase{ii,jj}),1);
        for s = 1:length(spk_phase{ii,jj})
            [Y,idx(s)] = min((phase{ii}-spk_phase{ii,jj}(s)).^2);
        end
        plot(t{ii}(idx(field_id{ii,jj}==1)),phase{ii}(idx(field_id{ii,jj}==1)),'.r');title(strcat('Cell ',num2str(jj),'-',F{jj}))
        plot([t{ii}(1) t{ii}(end)],[maxb maxb],'-b');
        plot([t{ii}(1) t{ii}(end)],[minb minb],'-b');
        if ~isempty(spk_phase{ii,jj}(field_id{ii,jj}==1))
            plot([t{ii}(1) t{ii}(end)],[circ_median(spk_phase{ii,jj}(field_id{ii,jj}==1)) circ_median(spk_phase{ii,jj}(field_id{ii,jj}==1))],'-r');
        end
        plot([t{ii}(1) t{ii}(end)],[pbins(im) pbins(im)],'--b');
        xlabel('time (sec)');ylabel('track angle');
        
        
        bmpImage = sprintf('%s%s%s%s',sessions{ii},strcat('PlaceFieldAngles','\'),F{jj}(1:end-2),'.bmp');
        %        epsImage = sprintf('%s%s%s%s',sessions{ii},strcat('PlaceFieldAngles','\'),F{jj}(1:end-2),'.eps');
        f = getframe(gcf);
        [pic, cmap] = frame2im(f);
        imwrite(pic,bmpImage,'bmp');
        %        saveas(1,epsImage,'epsc');
