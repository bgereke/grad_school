...
%    bmpfile = strrep(char(cluster(i)),'.t','.bmp')
   path = strcat('Path_',(char(cluster(i))))
   bmpfile = strrep(path,'.t','.bmp')
   ...
   if nargin == 2
       if (mod(i,cellspersheet) == 0) imwrite(J,strcat(Directory,'\',strrep(TTList,'.',''),imagedirectory,'path',num2str(i/cellspersheet),'.jpg')); end;
   else
...
if nargin == 2
   imwrite(J,strcat(Directory,'\',strrep(TTList,'.',''),imagedirectory,'pathlast','.jpg'));
else
   imwrite(J,strcat(Directory,'\',strrep(TTList,'.',''),imagedirectory,'_ordered','pathlast','.jpg'))
end
