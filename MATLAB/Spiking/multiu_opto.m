...
function [cells multiu] = multiu_opto(inFile,tbins,invh)
...
for kk=1:size(dirInfo,1)
    if dirInfo(kk).isdir
        if strcmp(dirInfo(kk).name,strcat('MultiU_Opto','\'))
            found = 1;
        end
    end
end
if found==0
    mkdir(savedir,strcat('MultiU_Opto','\'));
end
...
%     offspkr =  sum(exp(-0.5*deltaspkoff.*deltaspkoff*invh^2))./(sr{ii}*ones(1,size(deltatoff,1))*exp(-0.5*deltatoff.*deltatoff*invh^2));
    ...
    onhist = hist(onspks,tbins);
    offhist = hist(offspks,tbins);
    onthist = hist(ont,tbins);
    offthist = hist(offt,tbins);
    onrate = onhist./(onthist*0.029);
    offrate = offhist./(offthist*0.029);

    cells = [cells; offrate onrate];
    
    close all

    dt = tbins(2)-tbins(1);
    bar(tbins,offrate,1,'FaceColor',[1 1 1]);hold on
    bar(tbins+max(tbins)+dt/2,onrate,1,'FaceColor',[0 1 0])
    bar(tbins+2*max(tbins)+dt,offrate,1,'FaceColor',[1 1 1])
    xlabel('time (sec)');ylabel('spike rate (Hz)');
    title(strcat('Cell ',num2str(jj),'-',F{jj}))
    
    figImage = sprintf('%s%s%s%s',savedir,strcat('\MultiU_Opto','\'),F{jj}(1:end-2),'.fig');
    bmpImage = sprintf('%s%s%s%s',savedir,strcat('\MultiU_Opto','\'),F{jj}(1:end-2),'.bmp');
    saveas(gcf,figImage,'fig');
    saveas(gcf,bmpImage,'bmp');
    
%     deltamuon = [deltamuon; deltaspkon];
%     deltamuoff = [deltamuoff; deltaspkoff];

    ONHIST = ONHIST + onhist;
    OFFHIST = OFFHIST + offhist;
...
% %compute multiu time maps for light on/off
% onmur =  sum(exp(-0.5*deltamuon.*deltamuon*invh^2))./(sr{ii}*ones(1,size(deltaton,1))*exp(-0.5*deltaton.*deltaton*invh^2))/numCells;
% offmur =  sum(exp(-0.5*deltamuoff.*deltamuoff*invh^2))./(sr{ii}*ones(1,size(deltatoff,1))*exp(-0.5*deltatoff.*deltatoff*invh^2))/numCells;
% 
...
figImage = sprintf('%s%s%s%s',savedir,strcat('\MultiU_Opto','\'),'multiu.fig');
bmpImage = sprintf('%s%s%s%s',savedir,strcat('\MultiU_Opto','\'),'multiu.bmp');
saveas(gcf,figImage,'fig');
saveas(gcf,bmpImage,'bmp');
