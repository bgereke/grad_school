...
function [fields] = fields_opto(inFile,numperms,tbins,xbins,invh,kappa)
...
for kk=1:size(dirInfo,1)
    if dirInfo(kk).isdir
        if strcmp(dirInfo(kk).name,strcat('Fields_Opto','\'))
            found = 1;
        end
    end
end
if found==0
    mkdir(savedir,strcat('Fields_Opto','\'));
end
...
for jj = 1:numCells
    ...
    for ii = 1:counter-2
        ...
        for s=1:length(TS)
            ...
            if TTLs{ii}(s) == 0
                if s == 1
                    offspks = [offspks ts_locked(ts_locked<0 & ts_locked>=-30)'+30];
                    offt = [offt t_locked(t_locked<0&t_locked>=-30)+30];
                ...
                if s == 1
                    onspks = [onspks ts_locked(ts_locked<0 & ts_locked>=-30)'+30];
                    ont = [ont t_locked(t_locked<0&t_locked>=-30)+30];
                    phaseon = [phaseon spk_phase{ii,jj}(ts_locked<0 & ts_locked>=-30)'];
                    onphase = [onphase phase{ii}(t_locked<0&t_locked>=-30)];
                end
    ...
    delta_all = angle(exp(1j*repmat(spk_phase{ii,jj},1,size(xbins,2))).*conj(exp(1j*repmat(xbins,length(spk_phase{ii,jj}),1))));
    ...
    deltat_all = angle(exp(1j*repmat(phase{ii}',1,size(xbins,2))).*conj(exp(1j*repmat(xbins,length(phase{ii}),1))));
    ...
    tmap_all = (sr{ii}*ones(1,size(deltat_all,1))*exp(kappa*cos(deltat_all)));
    tmapon = (sr{ii}*ones(1,size(deltaton,1))*exp(kappa*cos(deltaton)));
    tmapoff = (sr{ii}*ones(1,size(deltatoff,1))*exp(kappa*cos(deltatoff)));
    allmap =  sum(exp(kappa*cos(delta_all)))./tmap_all;
    onmap =  sum(exp(kappa*cos(deltaspkon)))./tmapon;
    offmap =  sum(exp(kappa*cos(deltaspkoff)))./tmapoff;
%     allmap(tmap_all < 1) = nan;  
%     onmap(tmapon < 1) = nan;  
%     offmap(tmapoff < 1) = nan; 
    
    %detect dominant field boundaries
    meanmap = (offmap+onmap)/2;
    [mrate,midx] = max(meanmap);
    rotated = rotate([cos(xbins);sin(xbins)],xbins(midx)/(2*pi)*360);
    rbins = atan2(rotated(2,:)',rotated(1,:)');
    [~,sidx] = sort(rbins);
    rbins = rbins(sidx)';
    meanmap = meanmap(sidx);
    allmap = allmap(sidx);
    offmap = offmap(sidx);
    onmap = onmap(sidx);
    startidx = find(meanmap<0.1*mrate & rbins<0,1,'last');
    endidx = find(meanmap<0.1*mrate & rbins>0,1,'first');
    if isempty(startidx)
        [~,minidx] = findpeaks(-meanmap);
        startidx = max(minidx(minidx<0.5*length(xbins)));
    end
    if isempty(endidx)
        [~,minidx] = findpeaks(-meanmap);
        endidx = min(minidx(minidx>0.5*length(xbins)));
    end
    
    %normalize field size (0 to 1)
    nbins = linspace(0,1,60);
    nallmap = pchip(rbins(startidx:endidx),allmap(startidx:endidx),linspace(rbins(startidx),rbins(endidx),60));
    noffmap = pchip(rbins(startidx:endidx),offmap(startidx:endidx),linspace(rbins(startidx),rbins(endidx),60));
    nonmap = pchip(rbins(startidx:endidx),onmap(startidx:endidx),linspace(rbins(startidx),rbins(endidx),60));    

    subplot(1,2,1)
    plot(rbins,offmap,'-k','LineWidth',2);hold on
    plot(rbins,onmap,'-g','LineWidth',2);
    plot([rbins(startidx) xbins(startidx)],[0 mrate],'-k')
    plot([rbins(endidx) xbins(endidx)],[0 mrate],'-k')
    ...
    subplot(1,2,2)
    plot(nbins,noffmap,'-k','LineWidth',2);hold on
    plot(nbins,nonmap,'-g','LineWidth',2);
    xlim([0 1])
    ...
    fields = [fields; noffmap nonmap];
    
    bmpImage = sprintf('%s%s%s%s',savedir,strcat('\Fields_Opto','\'),F{jj}(1:end-2),'.bmp');
