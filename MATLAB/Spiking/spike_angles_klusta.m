...
function [peakRate, pathCoord] = spike_angles_klusta(inFile,numShanks)
...
%Get all spike times
%load and concatenate CSC time for all sessions
tt = [];
for ii = 1:numsessions
    xfile = [sessions{ii},channels{1},'.ncs'];
    [~,~,ttemp, ~, ~, ~] = loadEEG2(xfile);
    tt = [tt ttemp]; clear ttemp
end
% Load spike indices generated by KlustaKwik
Spks = LoadSpikes_klusta(sessions{1},numShanks);
numCells = length(Spks);
% Convert to times
for i = 1:numCells
   Spks{i} = tt(Spks{i}); 
end
ts = cell(numsessions,length(Spks));
...
for ii = 1:counter
    ...
    for kk=1:size(dirInfo,1)
        if dirInfo(kk).isdir
            if strcmp(dirInfo(kk).name,strcat('SpikeAngles','\'))
                found1 = 1;
            end
            if strcmp(dirInfo(kk).name,strcat('PlaceFieldAngles','\'))
                found2 = 1;
            end
        end
    end
    if found1==0
        mkdir(temp,strcat('SpikeAngles','\'));
    end
    if found2==0
        mkdir(temp,strcat('PlaceFieldAngles','\'));
    end
    
    %Get spikes from session
    starts = [1 find(diff(tt)>50)+1 length(tt)];
    for jj = 1:numCells
        ts{ii,jj} = Spks{jj}(Spks{jj}>=tt(starts(ii)) & Spks{jj}<tt(starts(ii+1)));
    end
...
%     y = -y + max(y)-min(y) + 2*max(y); % reflects posisitons so they are consistent with orientation in recording room
    ...
    for jj=1:numCells
        ...
        if isempty(ts{ii,jj}) % Empty cell in this session
...
for ii = 1:counter
    ...
    for jj=1:numCells
        ...
        plot(t{ii}(idx),phase{ii}(idx),'.r');title(strcat('Cell ',num2str(jj)))
        xlabel('time (sec)');ylabel('track angle');
        
        bmpImage = sprintf('%s%s%s%s',sessions{ii},strcat('SpikeAngles','\'),num2str(jj),'.bmp');
%         epsImage = sprintf('%s%s%s%s',sessions{ii},strcat('SpikeAngles','\'),num2str(jj),'.eps');
        f = getframe(gcf);
        [pic, cmap] = frame2im(f);
...
%         spk_temp = S(abs(angle(exp(1j*S).*conj(exp(1j*pbins(im)*ones(size(S))))))>vth & SV>vmin);
        ...
        plot(t{ii}(idx(field_id{ii,jj}==1)),phase{ii}(idx(field_id{ii,jj}==1)),'.r');title(strcat('Cell ',num2str(jj)))
        ...
        bmpImage = sprintf('%s%s%s%s',sessions{ii},strcat('PlaceFieldAngles','\'),num2str(jj),'.bmp');
