...
function [peakRate, V, pathCoord] = field_clust_check(inFile,scale_max)

sLength = 195; % Side length in cm
...
V = cell(counter,1);
minspkx = zeros(counter,numCells);
minspky = zeros(counter,numCells);
maxspkx = zeros(counter,numCells);
maxspky = zeros(counter,numCells);
...
for ii = 1:counter
    ...
    for jj=1:numCells
        ...
        if ~isa(S{jj},'ts') % Empty cell in this session
            ...
            x = x - x_cen;
            y = y - y_cen;
            spk_phase = atan2((spky - y_cen),(spkx - x_cen));
            spkxx = cos(spk_phase);
            spkyy = sin(spk_phase);
            v = [spkxx,spkyy];
            [field_id,mu,kappa,P_spkgmu] = movmf(v,1);
            [Y minidx] = min(spk_phase(field_id==1));
            [Y maxidx] = max(spk_phase(field_id==1));
            if ~isempty(Y)
            minspkx(ii,jj) = spkx(minidx);
            minspky(ii,jj) = spky(minidx);
            maxspkx(ii,jj) = spkx(maxidx);
            maxspky(ii,jj) = spky(maxidx);
            end
            % Calculate rate map
            map = ratemap(ts,spkx,spky,x,y,t,h,mapAxis);
        end
         rateMaps{ii,jj} = map;
...
for ii = 1:counter
    for jj=1:numCells
        ...
        drawnow; hold on        
        plot([0 minspkx(ii,jj)],[0,minspky(ii,jj)],'r')
        plot([0 maxspkx(ii,jj)],[0,maxspky(ii,jj)],'r')
