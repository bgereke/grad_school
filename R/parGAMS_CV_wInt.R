...
for(d in 1:length(unique(MYDATA$Day)))
{
  ...
  parLapplyLB(cl,f,function(f) 
  {
    ...
    mb<-"cr"
    rb<-"fs"
    fmod <- bam(MYDATA_d[,f] ~ s(Running_Speed,bs=mb) + s(Time,bs=mb,by=Session) + ti(Time,Running_Speed,k=15,bs=mb,by=Session) s(Running_Speed,Trial,bs=rb,xt=mb,m=1) + s(Time,Trial,bs=rb,xt=mb,m=1)

                ,method="fREML",family=gaussian(),data = MYDATA_d)
    
    #save model to disk
    setwd("/work/02592/bgereke/Data/GAMS_CV")
    filename<-paste0("VTbSIbS_","f",f,"_d",d,"_CICS.rds")
