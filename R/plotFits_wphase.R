...
#create new data
dgrid<-expand.grid(rsbins,tbins,pbins,unique(MYDATA$Session))
colnames(dgrid)<-c("runningspeed","time","Theta_Phase","session")
newdat<-with(MYDATA,data.frame(Running_Speed=dgrid$runningspeed,Time=dgrid$time,
                               Theta_Phase=dgrid$Theta_Phase,Session=dgrid$session))
...
ppidx<-which(!duplicated(newdat[,c('Theta_Phase')]))
prtintidx<-which(!duplicated(newdat[,c('Running_Speed','Time','Session')])) 
prpintidx<-which(!duplicated(newdat[,c('Running_Speed','Theta_Phase')]))
ptpintidx<-which(!duplicated(newdat[,c('Time','Theta_Phase','Session')])) 
used<-c(pi1idx,pi2idx,pi3idx,pridx,ptidx,ppidx,prtintidx,prpintidx,ptpintidx)
...
phasefit<-matrix(0,nrow=nb*nf,ncol=3)
timefit<-matrix(0,nrow=nb*nf*3,ncol=4)
rtintfit<-matrix(0,nrow=nb*nb*nf*3,ncol=5)
rpintfit<-matrix(0,nrow=nb*nb*nf,ncol=4)
tpintfit<-matrix(0,nrow=nb*nb*nf*3,ncol=5)
...
phasefit[,2]<-rep(freq,each=nb) #frequency
phasefit[,3]<-rep(pbins,times=nf) #Theta_Phase
...
rpintfit[,2]<-rep(freq,each=nb*nb) #frequency
rpintfit[,3]<-rep(rep(pbins,each=nb),times=nf) #Theta_Phase
rpintfit[,4]<-rep(rsbins,times=nb*nf) #running speed
tpintfit[,2]<-rep(unique(MYDATA$Session),each=nb*nb*nf) #session
tpintfit[,3]<-rep(rep(freq,each=nb*nb),times=3) #frequency
tpintfit[,4]<-rep(rep(pbins,each=nb),times=nf*3) #Theta_Phase
tpintfit[,5]<-rep(tbins,times=nb*nf*3) #time
...
psim<-matrix(0,nrow = nf*nb,ncol = ns)
timesim<-matrix(0,nrow = 3*nf*nb,ncol = ns)
rtintsim<-matrix(0,nrow = 3*nf*nb*nb,ncol = ns)
rpintsim<-matrix(0,nrow = nf*nb*nb,ncol = ns)
tpintsim<-matrix(0,nrow = 3*nf*nb*nb,ncol = ns)
fnum<-1:nf
...
for(f in 1:nf)
{
  ...
  filename<-paste0("Alz_f",fnum[f],"_POW.rds")
  ...
  ptpint1idx<-ptpintidx[newdat$Session[ptpintidx]=="1"]
  ptpint2idx<-ptpintidx[newdat$Session[ptpintidx]=="2"]
  ptpint3idx<-ptpintidx[newdat$Session[ptpintidx]=="3"]
  ...
  rpintidx<-((f-1)*nb*nb+1):(f*nb*nb)
  tpint1idx<-((f-1)*nb*nb+1):(f*nb*nb)
  tpint2idx<-((f-1)*nb*nb+1+nb*nb*nf):(f*nb*nb+nf*nb*nb)
  tpint3idx<-((f-1)*nb*nb+1+2*nb*nb*nf):(f*nb*nb+2*nf*nb*nb)
  ...
  #Theta_Phase
  want<-grep("s\\(Theta_Phase\\)",colnames(pred))
  ...
  want1<-grep("ti\\(Running\\_Speed\\,Time\\)\\:Session1",colnames(pred))
  want2<-grep("ti\\(Running\\_Speed\\,Time\\)\\:Session2",colnames(pred))
  want3<-grep("ti\\(Running\\_Speed\\,Time\\)\\:Session3",colnames(pred))
  ...
  #running speed/Theta_Phase interactions
  want1<-grep("ti\\(Running\\_Speed\\,Theta_Phase\\)",colnames(pred))
  rpintsim[rpintidx,]<-pred[prpintidx,want1] %*% t(sim[,want1])

  #time/Theta_Phase interactions
  want1<-grep("ti\\(Time\\,Theta_Phase\\)\\:Session1",colnames(pred))
  want2<-grep("ti\\(Time\\,Theta_Phase\\)\\:Session2",colnames(pred))
  want3<-grep("ti\\(Time\\,Theta_Phase\\)\\:Session3",colnames(pred))
  tpintsim[tpint1idx,]<-pred[ptpint1idx,want1] %*% t(sim[,want1]) #session 1
  tpintsim[tpint2idx,]<-pred[ptpint2idx,want2] %*% t(sim[,want2]) #session 2
  tpintsim[tpint3idx,]<-pred[ptpint3idx,want3] %*% t(sim[,want3]) #session 3
...
psimidx<-(max(rsimidx)+1):(max(rsimidx)+nrow(psim)) #Theta_Phase
timesimidx<-(max(psimidx)+1):(max(psimidx)+nrow(timesim)) #time by session
rtintsimidx<-(max(timesimidx)+1):(max(timesimidx)+nrow(rtintsim)) #time/running speed interaction by session
rpintsimidx<-(max(rtintsimidx)+1):(max(rtintsimidx)+nrow(rpintsim)) #runningspeed/Theta_Phase interaction
tpintsimidx<-(max(rpintsimidx)+1):(max(rpintsimidx)+nrow(tpintsim)) #time/Theta_Phase interaction

#get simultaneous CI's for main terms
sims<-rbind(isim,rsim,psim)
rm(isim,rsim,psim)
gc()
...
rm(rtintsim)
gc()
sims<-rbind(sims,rpintsim)
rm(rpintsim)
gc()
sims<-rbind(sims,tpintsim)
rm(tpintsim)
gc()
...
ggplot(rfit,aes(x=runningspeed,y=frequency,fill=fit,z=fit))+
  geom_raster()+scale_x_continuous(breaks=seq(round(minrs),round(maxrs),by=0.2),expand=c(0,0))+
  scale_y_continuous(breaks=seq(0,100,by=10),expand=c(0,0))+
  scale_fill_gradientn(colours = myPalette(250),limits = c(-cl,cl),oob=squish)+
...
ggsave("timesession_Alz.eps",device="eps",width=14,height=14,units="in")
